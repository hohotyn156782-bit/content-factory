"""Озвучка кусков. Два движка:
  • edge  — edge-tts (быстро, есть пословный тайминг через WordBoundary).
  • xtts  — Coqui XTTS v2 через изолированный venv (живее, голоса-дикторы; тайминг слов
            считаем пропорционально длине — куски короткие, на 1 фразу, этого достаточно).

Оба пути дают одинаковую структуру timed-кусков (audio, dur, start, end, words[]),
дальше общий код склеивает дорожку.
"""
import asyncio
import json
import os
...
def _xtts_timed(chunks: list[dict], voice: str, lang: str, workdir: pathlib.Path,
    ...
    # XTTS на CPU медленный — даём щедрый таймаут
    core.run([str(core.XTTS_VENV_PY), str(core.XTTS_WORKER), str(inp), str(outp)], timeout=1800)
...
def _moss_timed(chunks: list[dict], voice: str, lang: str, workdir: pathlib.Path,
    ...
    core.run([str(core.MOSS_VENV_PY), str(core.MOSS_WORKER), str(inp), str(outp)], timeout=1800)
    data = json.loads(outp.read_text(encoding="utf-8"))
    if data.get("error"):
...
def _groq_align(audio_path: pathlib.Path, lang: str = "ru") -> list[dict]:
...
def synthesize(chunks: list[dict], voice: str, rate: str, workdir: pathlib.Path,
               engine: str = "edge", lang: str = "ru", speed: float = 1.0) -> tuple[list[dict], pathlib.Path, float]:
    """Озвучить куски выбранным движком. Возвращает (timed_chunks, full_audio_path, total_dur).

    Для xtts при сбое — НЕ теряем ролик: откатываемся на edge-tts (быстрый и надёжный).
    """
...
if __name__ == "__main__":
    ...
    demo = [
        {"text": "Эта нейросеть делает за минуту то, на что уходил целый вечер.", "broll_query": "ai", "role": "hook"},
        {"text": "Просто опиши задачу словами — остальное она соберёт сама.", "broll_query": "computer", "role": "body"},
    ]
    eng = sys.argv[1] if len(sys.argv) > 1 else "edge"
    vc = sys.argv[2] if len(sys.argv) > 2 else ("Luis Moray" if eng == "xtts" else "ru-RU-DmitryNeural")
    wd = core.OUTPUT_DIR / "_voice_test"
    timed, audio, total = synthesize(demo, vc, "+8%", wd, engine=eng, lang="ru")
