...
def prompt_for(query: str, niche: dict, character: str = "") -> str:
    """EN-запрос сегмента → полноценный промпт фотореалистичной вертикальной сцены.
...
def _nvidia_flux_image(prompt: str, out: pathlib.Path, seed: int = 0) -> bool:
    """FLUX.1-dev через NVIDIA NIM (ключ NVIDIA_API_KEY, без карты). Нативная вертикаль 768x1344
    (9:16). Проверено 2026-06-16: работает, фотореализм выше Pollinations. Мульти-ключ + cooldown."""
    keys = [k.strip() for k in os.environ.get("NVIDIA_API_KEY", "").split(",") if k.strip()]
    if not keys:
        return False
    now = time.time()
    body = json.dumps({"prompt": prompt, "width": 768, "height": 1344, "steps": 30,
