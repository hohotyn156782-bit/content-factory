...
def _freeze_segments(video: str) -> tuple[list[float], bool]:
...
def _scene_cuts(video: str) -> tuple[list[float], bool]:
...
def _mean_volume(video: str) -> float:
...
def check_technical(video: str, niche: dict | None = None) -> dict:
    ...
    if not a:
        ...
        if _mean_volume(video) < -70:    # практически тишина → сбой озвучки/микса
    ...
    if vdur < 8:
        issues.append(f"подозрительно короткий ролик {vdur:.1f}с")
    # #1 верхняя граница: Shorts ≤60с. Ловит баг рендера (зацикленное аудио/дубль → 70-120с)
    max_s = (niche.get("max_seconds") if niche else None) or 70
    if vdur > max_s:
        issues.append(f"слишком длинный ролик {vdur:.1f}с — за пределами Shorts (≤60с)")
    frz, freeze_ok = _freeze_segments(video)
    ...
    cuts, cuts_ok = _scene_cuts(video)
...
def check_visual(video: str, vdur: float, workdir: pathlib.Path) -> dict:
    ...
    frames = _frames(video, vdur, workdir)
    if not frames:
        return {"ok": True, "issues": [], "checked": False, "note": "не извлёк кадры"}
    parts = [{"text": (
        "Это кадры из короткого видео. Найди ВИЗУАЛЬНЫЕ ДЕФЕКТЫ, из-за которых ролик нельзя публиковать: "
        "деформированные/искажённые лица, кривые руки, лишние или сросшиеся пальцы/конечности, "
        "неестественные тела, нечитаемый или искажённый текст/буквы-кракозябры на кадре, грубые AI-артефакты, "
        "полностью пустые/чёрные кадры. Косметику и стиль НЕ считай дефектом. "
        'Верни СТРОГО JSON: {"ok": true|false, "issues": ["короткое описание дефекта", ...]}. '
        "ok=false только при явных грубых дефектах.")}]
    for fp in frames:
    ...
    strict_parts[0] = {"text": parts[0]["text"] +
                       " ВАЖНО: верни ТОЛЬКО валидный JSON без markdown, без пояснений, без ```."}
...
def check(video: str, workdir: pathlib.Path | None = None, niche: dict | None = None) -> dict:
    ...
    vis = check_visual(video, tech["video_dur"], workdir)
