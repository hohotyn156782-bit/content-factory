...
def _snap_end(target_end: float, slot_start: float, bnds: list[float]) -> float | None:
    """Ближайшая граница chunk'а к target_end в окне ±SNAP_WINDOW, строго после slot_start.
    None → подходящей границы нет (fallback на SLOT_CYCLE-длину)."""
    best = None
    for b in bnds:
        if b <= slot_start + 0.05:               # слот должен иметь положительную длину
            continue
        if abs(b - target_end) <= SNAP_WINDOW and (best is None or abs(b - target_end) < abs(best - target_end)):
            best = b
    return best
...
def _pollinations_video(query: str, niche: dict, workdir: pathlib.Path, i: int) -> dict | None:
    ...
    for model in ("wan-fast", "wan"):
        url = (f"https://gen.pollinations.ai/video/{q}?model={model}"
               f"&width=608&height=1080&duration=5&nologo=true")
...
def fetch_for(timed_chunks: list[dict], niche: dict, workdir: pathlib.Path,
    ...
    cursor = 0.0
    for i, dur in enumerate(_slots(total, boundaries or None)):
        ...
        if mode in ("ai_images", "ai_video_hook", "depth_video"):
